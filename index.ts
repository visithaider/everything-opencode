...
export const EOCPlugin: Plugin = async ({ project, client, directory }) => {
...
`
    ...
    "session.deleted": async (input: unknown) => {
      await client.app.log({
        body: {
          service: PLUGIN_NAME,
          level: "info",
    ...
    "tool.execute.before": async (input: { tool: string }, output: { args: { command?: string; filePath?: string } }) => {
    ...
    "tool.execute.after": async (input: { tool: string; args?: { command?: string } }, output: { title: string; output: string; metadata: unknown }) => {
      if (input.tool === "bash") {
    ...
    "tui.toast.show": async (input: { args?: { message?: string } }) => {
      await client.app.log({
        body: {
          service: PLUGIN_NAME,
          level: "info",
