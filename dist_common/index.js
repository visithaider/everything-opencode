...
exports.ECCPlugin = void 0;
const plugin_1 = require("@opencode-ai/plugin");
const path = require("path");
const fs = require("fs");
const child_process_1 = require("child_process");
const PLUGIN_NAME = "ecc-universal";
const ECCPlugin = async ({ project, client, directory }) => {
    console.log(`[${PLUGIN_NAME}] Plugin initialized for ${directory}`);
    return {
        "session.created": async (input) => {
            await client.app.log({
                body: {
                    service: PLUGIN_NAME,
                    level: "info",
                    message: "ECC Plugin session started",
                    extra: {
                        project: directory,
                        directory
                    }
...
`;
        ...
        tool: {
            "run-tests": (0, plugin_1.tool)({
                description: "Run the project's test suite",
                args: {
                    coverage: plugin_1.tool.schema.boolean().optional(),
                    verbose: plugin_1.tool.schema.boolean().optional(),
                },
                async execute(args, context) {
                    ...
                    try {
                        const result = (0, child_process_1.execSync)(cmd, { cwd: directory, encoding: "utf-8" });
                        return result;
                    }
                    catch (error) {
                        const err = error;
            ...
            "check-coverage": (0, plugin_1.tool)({
                description: "Check test coverage meets the 80% threshold",
                args: {
                    threshold: plugin_1.tool.schema.number().optional().default(80),
                },
                async execute(args, context) {
                    const { directory } = context;
                    try {
                        ...
                        const result = (0, child_process_1.execSync)("npm test -- --coverage 2>&1", { cwd: directory, encoding: "utf-8" });
                        const output = result;
            ...
            "security-audit": (0, plugin_1.tool)({
                description: "Run security audit on dependencies",
                args: {
                    fix: plugin_1.tool.schema.boolean().optional(),
                },
                async execute(args, context) {
                    const { directory } = context;
            ...
            "typecheck": (0, plugin_1.tool)({
                ...
                async execute(args, context) {
                    ...
                    try {
                        const result = (0, child_process_1.execSync)("npx tsc --noEmit 2>&1", { cwd: directory, encoding: "utf-8" });
            ...
            "skill-create": (0, plugin_1.tool)({
                description: "Generate skills from git history",
                args: {
                    instincts: plugin_1.tool.schema.boolean().optional(),
                },
                async execute(args, context) {
                    const { directory } = context;
                    try {
                        const result = (0, child_process_1.execSync)('git log --all --pretty=format:"%s" -n 100', { cwd: directory, encoding: "utf-8" });
...
exports.ECCPlugin = ECCPlugin;
exports.default = exports.ECCPlugin;
